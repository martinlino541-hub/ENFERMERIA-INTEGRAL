...
import {
  Search, Eye, Users, Activity, Calendar, ChevronDown,
  FileText, X, Stethoscope, Phone, Briefcase, Heart,
  Clipboard, Clock, Download
} from 'lucide-react'
    ...
    try {
      const { data, error } = await supabase
        ...
        .select(`
          ...
          diagnosticos (
            id, codigo_cie10, nombre_cie10, tipo
          ),
          tratamientos (
            id, medicamento, cantidad, posologia
          )
        `)
    ...
    <div>
      <div className="page-header">
        <div className="page-title">
          ...
          <span className="page-title-badge">Matriz clínica</span>
        </div>
        ...
        <div className="card-body" style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
          <div className="search-bar" style={{ flex: 1, minWidth: 220 }}>
            ...
            <input
              ...
              placeholder="Buscar por nombre, cédula, diagnóstico..."
            />
            {search && (
              <button onClick={() => setSearch('')} style={{ border: 'none', background: 'none', cursor: 'pointer', color: 'var(--text-muted)' }}>
                <X size={14} />
              </button>
            )}
          </div>
          <select className="form-select" value={filterSexo}
      ...
      {loading ? (
        ...
        <div className="card" style={{ textAlign: 'center', padding: 60 }}>
          ...
          <div style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 4 }}>
            {search ? 'Intente con otros términos de búsqueda' : 'Cree una nueva atención médica para comenzar'}
          </div>
        </div>
      ) : (
                ...
                return (
                  <tr key={r.id}>
                    ...
                    <td>
                      {diags.length ? diags.map(d => (
                        <div key={d.id} style={{ marginBottom: 4 }}>
                          <span className={`table-badge ${d.tipo === 'definitivo' ? 'badge-def' : 'badge-pre'}`}>
                            {d.codigo_cie10}
                          </span>
                    ...
                    <td style={{ maxWidth: 180, fontSize: 12 }}>
                      {r.evolucion
                        ? <span title={r.evolucion}>{r.evolucion.substring(0, 60)}{r.evolucion.length > 60 ? '...' : ''}</span>
                        : '—'}
                    </td>
                    <td>
                    ...
                    <td style={{ whiteSpace: 'nowrap', fontSize: 12 }}>
                      {r.requiere_seguimiento ? (
                        <span style={{ color: 'var(--accent)', fontWeight: 600 }}>
                          📅 {r.proxima_consulta ? formatDate(r.proxima_consulta) : 'Pendiente'}
                        </span>
                      ) : (
            ...
            <div className="modal-header">
              <div>
                <div className="modal-title">
                  {selected.pacientes?.nombres} {selected.pacientes?.apellidos}
                </div>
                <div style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 2 }}>
                  Atención: {formatDate(selected.fecha_atencion)} · Cédula: {selected.pacientes?.cedula || 'N/D'}
                </div>
              </div>
              <button onClick={() => setSelected(null)} className="btn btn-ghost" style={{ padding: 8 }}>
                <X size={18} />
              </button>
            </div>
...
function DetailView({ r, formatDate }) {
  ...
  const Section = ({ title, children }) => (
    <div style={{ marginBottom: 20 }}>
      <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, color: 'var(--primary)', fontSize: 14, marginBottom: 10, paddingBottom: 6, borderBottom: '2px solid var(--accent)', display: 'inline-block' }}>
        {title}
      </div>
      {children}
    </div>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
      <div>
        ...
        <Section title="🔬 Signos Vitales">
          ...
          <Row label="Peso" value={r.peso ? `${r.peso} kg` : null} />
          <Row label="Talla" value={r.talla ? `${r.talla} cm` : null} />
          ...
          {(r.diagnosticos || []).length ? r.diagnosticos.map(d => (
            <div key={d.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: 8 }}>
              ...
              <div>
                ...
                <span className={`table-badge ${d.tipo === 'definitivo' ? 'badge-def' : 'badge-pre'}`} style={{ marginTop: 3 }}>
                  {d.tipo}
                </span>
              </div>
            </div>
        ...
        <Section title="💊 Tratamiento">
          {(r.tratamientos || []).length ? (
            <table style={{ width: '100%', fontSize: 12 }}>
              <thead>
                <tr>
                  {['Medicamento', 'Cantidad', 'Posología'].map(h => (
                    <th key={h} style={{ textAlign: 'left', padding: '4px 8px', color: 'var(--text-muted)', fontWeight: 700, fontSize: 11, textTransform: 'uppercase', borderBottom: '1px solid var(--border-light)' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
        ...
        {r.requiere_seguimiento && (
          <Section title="📅 Seguimiento">
            <div style={{ padding: 12, background: 'var(--accent-soft)', borderRadius: 'var(--radius)', border: '1px solid rgba(0,201,167,0.2)' }}>
              <div style={{ fontWeight: 600, color: 'var(--accent)' }}>Requiere seguimiento</div>
              {r.proxima_consulta && (
                <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 4 }}>
                  Próxima consulta: <strong>{formatDate(r.proxima_consulta)}</strong>
                </div>
              )}
            </div>
          </Section>
