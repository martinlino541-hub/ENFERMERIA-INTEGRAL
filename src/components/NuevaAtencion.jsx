    ...
    <div className="pos-relative" ref={ref}>
      ...
      {open && results.length > 0 && (
        <div className="cie10-dropdown">
          {results.map(r => (
            <div key={r.code} className="cie10-option" onMouseDown={() => { onSelect(r); setOpen(false) }}>
...
const emptyDiag = () => ({ id: Date.now(), codigo: '', nombre: '', tipo: 'presuntivo' })
const emptyTrat = () => ({ id: Date.now(), medicamento: '', cantidad: '', posologia: '' })
...
export default function NuevaAtencion({ onSaved }) {
  ...
  const [antecedentes, setAntecedentes] = useState({
    patologicos: '', alergicos: '', quirurgicos: '', familiares: '',
    gestas: '', partos: '', cesareas: '', abortos: '', planificacion: ''
  })

  const [motivo, setMotivo] = useState('')

  const [vitales, setVitales] = useState({
    pa: '', temperatura: '', saturacion: '', fr: '',
    consciencia: 'Alerta', peso: '', talla: '', perimetro: ''
  })
    ...
    try {
      ...
      if (generales.cedula) {
        const { data: existing } = await supabase
          .from('pacientes')
          .select('id')
          .eq('cedula', generales.cedula)
          .single()

        if (existing) {
          pacienteId = existing.id
          await supabase.from('pacientes').update({
            nombres: generales.nombres,
            apellidos: generales.apellidos,
            telefono: generales.telefono,
            edad: parseInt(generales.edad) || null,
            sexo: generales.sexo,
            ocupacion: generales.ocupacion,
          }).eq('id', pacienteId)
        }
      }
      ...
      const antData = {
        ...
        patologicos_personales: antecedentes.patologicos,
        alergicos: antecedentes.alergicos,
        quirurgicos: antecedentes.quirurgicos,
        patologicos_familiares: antecedentes.familiares,
        gestas: parseInt(antecedentes.gestas) || null,
        partos_vaginales: parseInt(antecedentes.partos) || null,
        cesareas: parseInt(antecedentes.cesareas) || null,
        abortos: parseInt(antecedentes.abortos) || null,
        metodo_planificacion: antecedentes.planificacion,
      }

      if (antExist) {
        await supabase.from('antecedentes').update(antData).eq('id', antExist.id)
      } else {
      ...
      const { data: atencion, error: atErr } = await supabase
        ...
        .insert({
          ...
          presion_arterial: vitales.pa,
          temperatura: parseFloat(vitales.temperatura) || null,
          saturacion: parseFloat(vitales.saturacion) || null,
          frecuencia_respiratoria: parseInt(vitales.fr) || null,
          estado_consciencia: vitales.consciencia,
          peso: parseFloat(vitales.peso) || null,
          talla: parseFloat(vitales.talla) || null,
          imc: parseFloat(imc) || null,
          perimetro_abdominal: parseFloat(vitales.perimetro) || null,
          evolucion,
          requiere_seguimiento: seguimiento,
          proxima_consulta: seguimiento && proximaConsulta ? proximaConsulta : null,
        })
        .select('id')
        .single()
      ...
      setAlert({ type: 'success', msg: `✓ Atención guardada correctamente para ${generales.nombres} ${generales.apellidos}.` })
      window.scrollTo({ top: 0, behavior: 'smooth' })
  ...
  return (
    <div>
      ...
      {alert && (
        <div className={`alert ${alert.type === 'success' ? 'alert-success' : 'alert-error'} mt-2`}
        ...
        <Section icon={User} title="Datos Generales del Paciente" subtitle="Información personal y de contacto">
          <div className="form-grid form-grid-3">
            <div className="form-group">
              ...
              <input className="form-input" value={generales.nombres}
                onChange={e => setGen('nombres', e.target.value)} placeholder="Nombres del paciente" />
            </div>
            <div className="form-group">
              <label className="form-label required">Apellidos</label>
              <input className="form-input" value={generales.apellidos}
                onChange={e => setGen('apellidos', e.target.value)} placeholder="Apellidos del paciente" />
            </div>
            <div className="form-group">
              <label className="form-label">Número de Cédula</label>
              <input className="form-input" value={generales.cedula}
                onChange={e => setGen('cedula', e.target.value)} placeholder="0912345678" maxLength={13} />
            </div>
            <div className="form-group">
              <label className="form-label">Teléfono</label>
              <input className="form-input" value={generales.telefono}
                onChange={e => setGen('telefono', e.target.value)} placeholder="0991234567" />
            </div>
            <div className="form-group">
              <label className="form-label">Edad</label>
              <input type="number" className="form-input" value={generales.edad}
                onChange={e => setGen('edad', e.target.value)} placeholder="Años" min={0} max={120} />
            </div>
            <div className="form-group">
              <label className="form-label">Sexo</label>
              <select className="form-select" value={generales.sexo}
                onChange={e => setGen('sexo', e.target.value)}>
            ...
            <div className="form-group col-span-2">
              ...
              <input className="form-input" value={generales.ocupacion}
                onChange={e => setGen('ocupacion', e.target.value)} placeholder="Cargo o función del trabajador" />
            </div>
          </div>
        </Section>
        ...
        <Section icon={Heart} title="Antecedentes Personales" subtitle="Historial médico personal" color="#D69E2E">
          <div className="form-grid form-grid-3">
            <div className="form-group">
              ...
              <textarea className="form-textarea" value={antecedentes.patologicos}
                onChange={e => setAnt('patologicos', e.target.value)}
                placeholder="HTA, DM2, asma, hipotiroidismo..." />
            </div>
            <div className="form-group">
              <label className="form-label">Antecedentes Alérgicos</label>
              <textarea className="form-textarea" value={antecedentes.alergicos}
                onChange={e => setAnt('alergicos', e.target.value)}
                placeholder="Alergias a medicamentos, alimentos, etc..." />
            </div>
            <div className="form-group">
              <label className="form-label">Antecedentes Quirúrgicos</label>
              <textarea className="form-textarea" value={antecedentes.quirurgicos}
                onChange={e => setAnt('quirurgicos', e.target.value)}
                placeholder="Cirugías previas y año..." />
            </div>
          </div>
        </Section>
        ...
        <Section icon={Users} title="Antecedentes Patológicos Familiares" subtitle="Historial de enfermedades en familia directa" color="#744210">
          <div className="form-group">
            ...
            <textarea className="form-textarea" value={antecedentes.familiares}
              onChange={e => setAnt('familiares', e.target.value)}
              placeholder="Padre: HTA, Madre: DM2, Hermano: cardiopatía..." style={{ minHeight: 70 }} />
          </div>
        </Section>
          ...
          <Section icon={Baby} title="Antecedentes Ginecobstétricos" subtitle="Solo aplica para pacientes femeninas" color="#97266D">
            <div className="form-grid form-grid-5">
              <div className="form-group">
                ...
                <input type="number" className="form-input" value={antecedentes.gestas}
                  onChange={e => setAnt('gestas', e.target.value)} placeholder="0" min={0} />
              </div>
              <div className="form-group">
                <label className="form-label">Partos Vaginales</label>
                <input type="number" className="form-input" value={antecedentes.partos}
                  onChange={e => setAnt('partos', e.target.value)} placeholder="0" min={0} />
              </div>
              <div className="form-group">
                <label className="form-label">Cesáreas</label>
                <input type="number" className="form-input" value={antecedentes.cesareas}
                  onChange={e => setAnt('cesareas', e.target.value)} placeholder="0" min={0} />
              </div>
              <div className="form-group">
                <label className="form-label">Abortos</label>
                <input type="number" className="form-input" value={antecedentes.abortos}
                  onChange={e => setAnt('abortos', e.target.value)} placeholder="0" min={0} />
              </div>
              <div className="form-group">
                <label className="form-label">Método de Planificación</label>
                <select className="form-select" value={antecedentes.planificacion}
                  onChange={e => setAnt('planificacion', e.target.value)}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px,1fr))', gap: 12 }}>
            {[
              ...
              <div key={key} className="vital-card">
                ...
                <input
                  value={vitales[key]}
                  onChange={e => setVit(key, e.target.value)}
          ...
          <div className="form-group mt-3">
            ...
            <div className="consciousness-grid mt-1">
              {['Alerta', 'Somnoliento', 'Obnubilación', 'Estupor', 'Coma'].map(c => (
                <button key={c}
                  className={`consciousness-btn ${vitales.consciencia === c ? 'selected' : ''}`}
                  onClick={() => setVit('consciencia', c)}>
                  {c}
                </button>
              ))}
            </div>
        ...
        <Section icon={Clipboard} title="Evolución del Paciente" subtitle="Descripción clínica y seguimiento" color="#553C9A">
          <div className="form-group">
            ...
            <textarea className="form-textarea" value={evolucion}
              ...
              placeholder="Descripción de la evolución clínica, hallazgos al examen físico, plan de manejo..."
              style={{ minHeight: 120 }} />
          </div>
        </Section>
              ...
              <div key={diag.id} className="diagnostico-row">
                <div className="form-group">
                  <input className="form-input" value={diag.codigo}
                    onChange={e => setDiagField(diag.id, 'codigo', e.target.value.toUpperCase())}
                    placeholder="J45.9" style={{ fontWeight: 700, fontFamily: 'var(--font-display)' }} />
                </div>
                <div className="pos-relative">
                  <CIE10Search
                    value={diag.nombre}
                    onChange={v => setDiagField(diag.id, 'nombre', v)}
                    onSelect={r => {
                      setDiagField(diag.id, 'nombre', r.name)
                      setDiagField(diag.id, 'codigo', r.code)
                    }}
                ...
                <div style={{ display: 'flex', justifyContent: 'center' }}>
                  <button
                    className={`tipo-badge ${diag.tipo}`}
                    onClick={() => setDiagField(diag.id, 'tipo', diag.tipo === 'definitivo' ? 'presuntivo' : 'definitivo')}
                  >
                    {diag.tipo === 'definitivo' ? '● Definitivo' : '◌ Presuntivo'}
                  </button>
                </div>
                {diagnosticos.length > 1 && (
                  <button className="btn-danger-ghost" onClick={() => removeDiag(diag.id)}>
                    <Trash2 size={15} />
                  </button>
                )}
              </div>
          ...
          <div className="section-stack">
            {tratamientos.map((trat) => (
              <div key={trat.id} className="tratamiento-row">
                <input className="form-input" value={trat.medicamento}
                  onChange={e => setTratField(trat.id, 'medicamento', e.target.value)}
                  placeholder="Paracetamol 500mg" />
                <input className="form-input" value={trat.cantidad}
                  onChange={e => setTratField(trat.id, 'cantidad', e.target.value)}
                  placeholder="10 tab." />
                <input className="form-input" value={trat.posologia}
                  onChange={e => setTratField(trat.id, 'posologia', e.target.value)}
                  placeholder="1 tab. cada 8h por 3 días" />
                {tratamientos.length > 1 && (
                  <button className="btn-danger-ghost" onClick={() => removeTrat(trat.id)}>
                    <Trash2 size={15} />
                  </button>
                )}
              </div>
            ))}
            <button className="btn-add" onClick={addTrat}>
        ...
        <div className="seguimiento-panel">
          ...
          {seguimiento && (
            <div className="form-group" style={{ flex: 1, maxWidth: 240 }}>
              ...
              <input type="date" className="form-input" value={proximaConsulta}
