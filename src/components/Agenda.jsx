...
export default function Agenda() {
  ...
  const fetchCitas = useCallback(async () => {
    ...
    try {
      ...
      // Solo citas programadas (proxima_consulta) del mes
  ...
  const citasDelDia = (d) => {
    const f = fechaStr(d)
    return citas.filter(c => c.proxima_consulta === f)
  }
  ...
  return (
    <div>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <div>
            <div className="page-title">
              <Calendar size={22} />
              Agenda de Citas
              <span className="page-title-badge">Programadas</span>
            </div>
            <div className="page-subtitle">Citas de seguimiento programadas por fecha</div>
          </div>
          <button
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 12, marginBottom: 20 }}>
        {[
          { label: 'Citas programadas en el mes', value: totalCitasMes, color: '#2F855A', bg: '#F0FFF4' },
            ...
            {loading ? (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 4 }}>
                {celdas.map((d, i) => {
                  ...
                  const tieneCitas = citasD.length > 0
                  ...
                  return (
                    <div key={d}
                      ...
                      style={{
                        ...
                        padding: '6px 8px',
                        cursor: 'pointer',
                        background: seleccionado ? 'var(--primary)' : hoyFlag ? '#EBF8FF' : tieneCitas ? 'rgba(0,201,167,0.08)' : 'var(--surface-2)',
                        border: seleccionado ? '2px solid var(--primary)' : hoyFlag ? '2px solid var(--secondary)' : tieneCitas ? '1.5px solid var(--accent)' : '1.5px solid var(--border-light)',
                        transition: 'all 0.15s',
                      }}
                    >
                      <div style={{
                        fontFamily: 'var(--font-display)',
                        fontWeight: hoyFlag || seleccionado || tieneCitas ? 700 : 500,
                        fontSize: 14,
                        color: seleccionado ? 'white' : hoyFlag ? 'var(--secondary)' : tieneCitas ? 'var(--accent)' : 'var(--text-primary)',
                        marginBottom: 4,
                      }}>{d}</div>

                      {tieneCitas && (
                        <div style={{
                          background: seleccionado ? 'rgba(255,255,255,0.3)' : 'var(--accent)',
                          color: 'white', fontSize: 10, fontWeight: 700,
                          padding: '2px 6px', borderRadius: 8, display: 'inline-block',
                        }}>
                          {citasD.length} cita{citasD.length > 1 ? 's' : ''}
                        </div>
                      )}
                    </div>
                  )
